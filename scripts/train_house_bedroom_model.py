"""CLI entrypoint for the dedicated grouped house-bedroom classifier."""
...
from lesotho_property_ai.vision import train_house_bedroom_model
...
def _resolve_default_input_csv() -> Path:
    ...
    raise FileNotFoundError(
        "No curated image dataset was found. Expected reviewed or candidate image CSV artifacts."
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the dedicated bedroom-class classifier on the reviewed house image dataset."
    )
    parser.add_argument(
        "--input-csv",
        default=None,
        help="Optional curated image-level dataset CSV. If omitted, the reviewed house image dataset is used when available.",
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> None:
    ...
    result = train_house_bedroom_model(
    ...
    print("House bedroom training complete")
